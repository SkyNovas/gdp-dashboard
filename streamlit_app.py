...
if "Desarrollo (DEV)" in selected_environments:
    ...
    diferencia = eco_actual - dev_annual
    diferencia_porcentaje = (abs(diferencia) / eco_actual) * 100
    
    # Crear DataFrame
    df_pie = pd.DataFrame({
        'Categoría': ['Ecosistema Actual', 'Desarrollo (DEV)'],
        'Valor': [eco_actual, dev_annual],
        'Porcentaje': [f'{(eco_actual/(eco_actual+dev_annual))*100:.1f}%', 
                      f'{(dev_annual/(eco_actual+dev_annual))*100:.1f}%']
    })
    
    # Crear gráfico con estilo de imagen
    fig = px.pie(
        df_pie,
        names='Categoría',
        values='Valor',
        ...
        color_discrete_sequence=['#484ae8', '#66BB6A'],
    ...
    # Añadir fórmula de diferencia porcentual
    fig.add_annotation(
        x=0.5,
        y=-0.3,
        text=(
            f"Diferencia Porcentual = |{dev_annual:,.2f} - {eco_actual:,.2f}| / {eco_actual:,.2f} × 100\n"
